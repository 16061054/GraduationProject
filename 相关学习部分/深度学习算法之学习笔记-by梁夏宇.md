...
### 训练集，验证集与测试集
　　在机器学习中，通常把样本划分为训练集、验证集和测试集这几个部分。训练集用于训练模型，验证集用于选定最终模型，测试集用于对选定的模型的预测结果进行评估。值得注意的是，现代深度学习的一个趋势是在训练集和测试集分布不匹配的情况下进行模型训练，也就是说训练集和测试集可能不是来自同一分布，这时最好保证验证集和测试集来自同一分布。

　　对于较小规模的数据集（例如包含1万条数据），通常是70%验证集、30%测试集，或者60%训练集、20%验证集和20%测试集。对于规模较大的数据（例如百万条数据），只要保留足够数量的数据用于测试和验证即可，对分配比例没有要求，比如100万条数据，其中1万条作为验证集，1万条作为测试集，剩余部分作为训练集。

　　偏差和方差是研究衡量数据集的指标（百分数），其中偏差用于衡量训练集误差，方差用于衡量验证集误差。高偏差发生时，意味着模型没有得到很好的训练、对训练数据的拟合度不高；高方差发生时，意味着验证集未能充分发挥交叉验证作用。高偏差与高方差是相对于最优误差（也称为贝叶斯误差）而言的，例如当最优误差为15%，训练误差是15%而验证误差是16%，这时偏差和方差均很低。

　　偏差较高的应对方法是修改模型（增加隐藏层或隐藏单元），方差较高的应对方法是采集更多数据或通过正则化减少过拟合。

### 过拟合与正则化
　　深度学习可能存在过拟合问题（过拟合的定义在机器学习部分），此时会出现高方差，这时优先考虑正则化。正则化的定义同样在机器学习部分有提过，但该定义是基于数值的，基于向量的正则化见 http://www.ai-start.com/dl2017/html/lesson2-week1.html 的1.4节（L2正则化）和1.6节（dropout正则化，计算机视觉领域常用）。

### 归一化输入
　　归一化输入可以加快神经网络的训练速度。归一化需要两个步骤，第一步是零均值化，第二步是归一化方差。归一化起到的作用与上面的《多变量线性回归》一节提到的特征缩放是一样的。

![avatar](img/归一化的作用.png)

### 梯度消失和梯度爆炸
　　训练神经网络（尤其是深度神经网络）时，导数或坡度有时会变得非常大或非常小（甚至是指数级变化），这就是梯度爆炸和梯度消失。导数的剧烈变化会严重影响梯度下降算法的表现。

　　针对该问题的一个方法是合理设置初始化权重。

### 梯度检验
　　梯度检验是用于调试或检验反向传播的实施是否正确的方法，见 http://www.ai-start.com/dl2017/html/lesson2-week1.html 1.13梯度检验。值得注意的是，dropout和梯度检验不能同时使用，因为dropout破坏了原有的神经网络（随机抹除了一些神经元）。

### 优化算法
　　Mini-batch梯度下降：在进行超大规模（比如千万级别）样本的训练时，把训练集分割为小一点的训练子集（也称为mini-batch）。进行梯度下降训练时，基于训练子集（而非整个训练集）进行一步步的梯度下降。

　　指数加权平均数与偏差修正：构建一些优化算法（比梯度下降算法更快）需要用到。基于此，动量梯度下降法计算梯度的指数加权平均数，并用该梯度更新权重。RMSprop算法可以同时加快多个方向上的学习。Adam算法结合了上述两种算法，有效适用于不同的神经网络。

　　通过学习率衰减的方法可以加快学习算法，因为如果学习率不变，那么算法不会精确收敛，而是在局部最优解附近摆动。

　　上面多次提到梯度下降算法得到的是局部最优解，实际上这只是在低维空间才存在获取多个局部最优解之一的可能性，在高维空间几乎不存在多个局部最优解的可能。

### 超参数调试
　　对于多个超参数（学习速率、学习衰减率等），很难确定哪个更重要，这时最好以超参数为维度，在合适的范围内随机选择点，逐步逼近表现最好的超参数配置区间。在实际训练时，如果拥有的计算资源较多，可以采取并行训练多个模型的方式以确定最优超参数组合；反之可以对一个模型用不同的超参数组合多次训练。

### Batch归一化
　　batch归一化与前面提到的归一化实际内容差不多，不过这里的归一化不仅仅用于输入，还可以用于隐藏层。batch归一化的一个好处是通过归一化所有的输入特征，可以获得相似范围的值，从而加速学习过程；另外一个好处是是权重比网络更滞后或更深层，这意味着网络受输入值改变的影响减小了，受输入分布改变的影响更小，网络变得更稳定。此外，batch归一化在一定程度上起到了正则化的效果，可以防止过拟合。

### Softmax回归
　　softmax回归可以实现多分类。与hardmax相比，softmax更温和——hardmax给出的分类结果向量是类似于(1,0,0,0,0,0)的，而softmax给出的是属于不同分类的概率。

### 深度学习框架
　　现在构建深度神经网络一般不会从0开始自己搭，而是使用深度学习框架。常用框架有TensorFlow、Keras、Torch等。

## 循环神经网络RNN

### 循环神经网络
　　循环序列模型即RNN，主要应用在语音识别、自然语言处理等涉及顺序（尤其是时间顺序）的学习领域。循环神经网络的特征在于，在每一个时间步中，循环神经网络传递一个值到下一个时间步中用于计算，并且神经网络的每一个隐藏层都可以预测输出（但不一定都有输出）。对于不同的场景，RNN具有一对一、一对多、多对一和多对多几种结构。RNN常用tanh作为激活函数。

### 通过时间的反向传播
　　反向传播的损失函数也叫交叉熵损失函数，和前面二分类的损失函数很像，这是关于单个位置（某个时间步）的预测值的损失函数。整个序列的损失函数是从第一步开始依次计算每一个时间步的损失函数并求和得到。反向传播与此相似，需要从最后一步开始，逐步往前求偏导数——这里实际上由模型自动完成。

### 梯度消失
　　基本RNN算法很难通过反向传播，把后面层的计算误差传导到前面层、从而影响前面层的计算，这导致它不擅长处理长期依赖问题（比如一句话的单词之间存在的关联）。

### 门控循环单元GRU
　　GRU改变了RNN的隐藏层，使其可以更好地捕捉深层连接，并改善了梯度消失问题。

　　GRU为每个隐藏层增加了新变量c，代表记忆细胞cell，在每个时间步记忆细胞都可能会被更新，而更新时机由更新门决定。更新门通过S型函数产生更新概率，在关联较小时更新概率极低，在关联较大时更新概率极高。为此需要增加一个与更新门对应的相关门。

![avatar](img/门控循环单元GRU.png)

### 长短期记忆LSTM
　　LSTM模型的原理与GRU相似，但它增加了一个门且对GRU的两个门进行了修改；另外LSTM还可以通过“偷窥孔廉价”获取上一个记忆细胞的值，用于当前更新判断。实际上LSTM出现得比GRU更早，也更强大和灵活；而GRU更简单且适应规模更大的问题。

![avatar](img/长短期记忆LSTM.png)

### 双向循环神经网络BRNN
　　双向循环神经网络模型在序列某点处不仅可以获取之前的信息，还可以获取未来的信息。BRNN不仅包括正向连接还包括反向连接，并且这里的反向连接不是指反向传播，而是指与正向传播极为相似的映射过程。当正向连接计算完成，反向连接即可开始计算；正反向连接都计算完成，即可计算预测结果。

　　BRNN的缺点是需要完整的数据序列才能进行预测。

![avatar](img/双向循环神经网络BRNN.png)

### 深层循环神经网络DRNN
　　深层循环神经网络把RNN的多个层堆叠在一起构建更深的模型。DRNN可以增强模型的表达能力，但需要很多的计算资源。

![avatar](img/深层循环神经网络BRNN.png)